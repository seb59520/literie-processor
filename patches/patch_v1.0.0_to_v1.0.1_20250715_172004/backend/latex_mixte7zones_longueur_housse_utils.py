...
def get_latex_mixte7zones_longueur_housse_value(longueur, matiere_housse):
    """
    Retourne la valeur de longueur housse LATEX MIXTE 7 ZONES selon la longueur détectée et la matière housse.
    Args:
    ...
    try:
        ...
        json_path = os.path.join(script_dir, "Référentiels", "latex_mixte7zones_longueur_housse.json")
        ...
        for entry in data:
            if entry["LONGUEUR"] == longueur:
                if matiere_housse == "LUXE 3D" or matiere_housse == "TENCEL LUXE 3D":
                    ...
                    return None
        print(f"Longueur {longueur} non trouvée dans le référentiel LATEX MIXTE 7 ZONES LONGUEUR HOUSSE")
        return None
    except FileNotFoundError:
        print(f"Fichier référentiel LATEX MIXTE 7 ZONES LONGUEUR HOUSSE non trouvé: {json_path}")
        return None
    except Exception as e:
        print(f"Erreur lors de la lecture du référentiel LATEX MIXTE 7 ZONES LONGUEUR HOUSSE: {e}")
        return None 