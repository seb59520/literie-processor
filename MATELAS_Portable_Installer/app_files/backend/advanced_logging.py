...
class MatelasLogger:
    ...
    def __init__(self, log_dir: str = "logs"):
        # Assurer que le répertoire de logs est dans un emplacement accessible
        if os.path.isabs(log_dir):
            self.log_dir = Path(log_dir)
        else:
            # Utiliser le répertoire utilisateur ou le répertoire du script
        ...
        # Créer le répertoire en gérant les permissions
        try:
            self.log_dir.mkdir(parents=True, exist_ok=True)
        except PermissionError:
            # Fallback vers le répertoire temporaire utilisateur
            import tempfile
            temp_dir = Path(tempfile.gettempdir()) / "matelas_logs"
            temp_dir.mkdir(exist_ok=True)
            self.log_dir = temp_dir
            print(f"⚠️ Utilisation du répertoire temporaire pour les logs: {self.log_dir}")
