...
class LLMProviderManager:
    ...
    def call_llm(self, prompt: str, file_name: str = "unknown", file_size: int = 0, session_id: str = None, **kwargs) -> Dict[str, Any]:
        ...
        logger.info(f"Appel LLM avec provider: {self.current_provider} (modèle: {model_used})")
        
        # Faire l'appel API
        result = provider_instance.call_llm(prompt, **kwargs)
        
        # Calculer le temps de traitement si pas déjà fourni
        if 'processing_time' not in result:
            result['processing_time'] = time.time() - start_time
        
        # Enregistrer les coûts si l'appel a réussi
        if result.get('success', False) and 'usage' in result:
            try:
                ...
                # Ajouter les informations de coût au résultat
                costs = cost_tracker.calculate_cost(self.current_provider, model_used, result['usage'])
                result['cost_info'] = costs
                
            except Exception as e:
                logger.warning(f"Erreur enregistrement coût: {e}")
        else:
            # Enregistrer l'échec
            try:
                cost_tracker.record_api_call(
                ...
                logger.warning(f"Erreur enregistrement échec: {e}")
