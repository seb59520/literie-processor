#!/usr/bin/env python3
"""
Patch pour le client auto_updater.py pour envoyer les informations de télémétrie
Ce script modifie automatiquement le backend/auto_updater.py pour inclure la télémétrie
"""
...
def create_telemetry_enhanced_client():
    """Crée une version améliorée du client avec télémétrie"""
    
    print("🔧 CRÉATION DU CLIENT AVEC TÉLÉMÉTRIE")
    print("=" * 50)
    
    # Lire le client existant
    original_client = Path("backend/auto_updater.py")
    if not original_client.exists():
        print("❌ Fichier backend/auto_updater.py non trouvé")
        return False
    
    # Créer une sauvegarde
    backup_path = Path(f"backend/auto_updater_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.py")
    shutil.copy2(original_client, backup_path)
    print(f"💾 Sauvegarde créée: {backup_path}")
    
    # Contenu du nouveau client avec télémétrie
    enhanced_client_code = '''#!/usr/bin/env python3
"""
...
'''
    ...
    # Écrire le nouveau client
    enhanced_client_path = Path("backend/auto_updater_with_telemetry.py")
    enhanced_client_path.write_text(enhanced_client_code, encoding='utf-8')
    
    print(f"✅ Client avec télémétrie créé: {enhanced_client_path}")
    
    # Demander s'il faut remplacer l'original
    response = input("\n❓ Remplacer le client original par la version avec télémétrie? [o/N]: ").strip().lower()
    
    if response in ['o', 'oui', 'y', 'yes']:
        shutil.copy2(enhanced_client_path, original_client)
        print(f"✅ Client original remplacé")
        print(f"💾 Ancienne version sauvegardée: {backup_path}")
    else:
        print(f"ℹ️ Nouvelle version disponible: {enhanced_client_path}")
        print(f"   Pour l'utiliser, copiez-la manuellement vers {original_client}")
    
    return True

if __name__ == "__main__":
    print("🔧 PATCH CLIENT MATELAS AVEC TÉLÉMÉTRIE")
    print("=" * 50)
    print()
    print("Ce script va créer une version améliorée du client auto_updater.py")
    print("qui enverra les informations du poste au serveur d'administration.")
    print()
    print("Informations collectées (de manière anonyme):")
    print("• Nom du poste et utilisateur")
    print("• Système d'exploitation et version")
    print("• Version actuelle de MATELAS")
    print("• Adresse IP locale")
    print("• ID unique du client (généré automatiquement)")
    print()
    
    response = input("Continuer? [O/n]: ").strip().lower()
    
    if response not in ['n', 'no', 'non']:
        success = create_telemetry_enhanced_client()
        
        if success:
            print("\n🎉 PATCH APPLIQUÉ AVEC SUCCÈS!")
            print()
            print("📋 PROCHAINES ÉTAPES:")
            print("1. Testez le nouveau client avec télémétrie")
            print("2. Redémarrez votre serveur d'administration avec télémétrie:")
            print("   python3 online_admin_interface/enhanced_admin_with_telemetry.py")
            print("3. Lancez l'application client pour voir les informations remontées")
            print("4. Consultez http://localhost:8090/admin/clients")
        else:
