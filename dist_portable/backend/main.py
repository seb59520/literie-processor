...
try:
    ...
    try:
        temp_log = Path(tempfile.gettempdir()) / "matelas_app_debug.log"
        file_handler = logging.FileHandler(str(temp_log), mode='w', encoding='utf-8')
        file_handler.setLevel(logging.INFO)
        formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')
        file_handler.setFormatter(formatter)
        logger.addHandler(file_handler)
        logger.info(f"Logs sauvegardés dans dossier temporaire: {temp_log}")
    except Exception as e2:
        logger.error(f"Impossible de créer un fichier de log: {e2}")
