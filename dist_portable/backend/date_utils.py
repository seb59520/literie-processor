...
def calculate_production_weeks(semaine_actuelle: int, annee_actuelle: int, 
    ...
    - Si matelas + sommiers : sommiers S+1, matelas S+2
    ...
    if has_matelas and has_sommiers:
        ...
        # Sommiers en S+1, matelas en S+2
        semaine_sommiers, annee_sommiers = next_week(semaine_actuelle, annee_actuelle)
        semaine_matelas, annee_matelas = next_week(semaine_sommiers, annee_sommiers)
...
def _generate_recommendation_text(has_matelas: bool, has_sommiers: bool,
    ...
    if has_matelas and has_sommiers:
        return f"Recommandation : Sommiers en S{semaine_sommiers} ({annee_sommiers}), Matelas en S{semaine_matelas} ({annee_matelas})"
    elif has_matelas:
