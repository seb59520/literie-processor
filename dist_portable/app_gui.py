        ...
        <pre style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; overflow-x: auto;">
{
    ...
    def show_eula(self):
        ...
        # Chercher le fichier EULA relativement au fichier de l'application
        if getattr(sys, 'frozen', False):
            # Si exécutable PyInstaller
            eula_file = os.path.join(sys._MEIPASS, "EULA.txt")
        else:
            # Si script Python normal
            eula_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "EULA.txt")
        
        if not os.path.exists(eula_file):
            QMessageBox.warning(self, "Fichier manquant", f"Le contrat d'utilisation (EULA.txt) est introuvable à l'emplacement: {eula_file}.")
            return
        try:
    ...
    def check_eula_acceptance(self):
        ...
        if not os.path.exists(eula_file):
            QMessageBox.critical(self, "Fichier manquant", f"Le contrat d'utilisation (EULA.txt) est introuvable à l'emplacement: {eula_file}. L'application va se fermer.")
            sys.exit(1)
        try:
            with open(eula_file, 'r', encoding='utf-8') as f:
                eula_content = f.read()
